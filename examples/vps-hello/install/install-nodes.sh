#!/usr/bin/env bash
# wget -qO- https://raw.githubusercontent.com/naiemk/vibed-infra/main/examples/vps-hello/install/install-nodes.sh | bash
set -euo pipefail
_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
if [[ -f "$_SCRIPT_DIR/packager.sh" ]]; then
  # shellcheck source=packager.sh
  source "$_SCRIPT_DIR/packager.sh"
  _infra_run_install nodes
fi
...
export PACKAGER_RAW PACKAGECONFIG_URL PRODUCT_RAW INFRA_PROFILE=nodes INSTALL_DIR="${INSTALL_DIR:-.}"

#!/usr/bin/env bash
# Digest-gated pull + compose recreate when NODES_AUTO_UPDATE is on.
set -euo pipefail
...
run_nodes_update() {
  ...
  name="${WORKER_CONTAINER_NAME:-hello-worker-heartbeat}"
  ...
  PULL=0 "$SCRIPT_DIR/start-hello-nodes.sh"

...
def _dns_skill(product_name: str, sites: list) -> str:
    tmpl = (
    ...
    for site in sites:
        host = site["host"]
        lines.append(f"| `{host}` | A | `{{{{VPS_IP}}}}` |")
        for alias in site.get("aliases") or []:
            lines.append(f"| `{alias}` | A | `{{{{VPS_IP}}}}` |")
...
def build_dist(
    ...
    *,
    ...
    _write(out_dir / "DNS-SKILL.md", _dns_skill(meta.get("productName") or conf.get("name") or "app", sites))
